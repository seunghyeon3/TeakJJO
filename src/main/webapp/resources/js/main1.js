...
function getList(data, index) {
    return `<form action="reserve.do" method="post" class="movie-informaion" id="${index}">
    ...
    <div>
        ...
        <div class="like-reserve-wrapper">
            <div><button type="button" class="movie-like"><img class="likeButton"
                        src="resources/img/heart.png"><span>${data.like}</span></button>
            </div>
            <button type="button" class="reserveButton" id="reserve${index}"><img src="resources/img/reserve.PNG"></button>
        </div>
    </div>
